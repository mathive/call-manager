...
class WhatsAppBlockerService : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        ...
        // Detect missed call notification to restore ringer/DND immediately
        if (isBlockingWhatsAppCall && text.contains("missed call", ignoreCase = true)) {
            Log.d("WhatsAppBlocker", "Missed call detected. Restoring ringer early.")
            restoreRinger()
            return 
        }
        ...
        if (isCall) {
            ...
            if (callerIdentity.isNotEmpty()) {
                ...
                // Decision logic
                val shouldBlock = isBlockedManually || !isKnown
                ...
                if (shouldBlock) {
                    ...
                    if (prefs.getBoolean("edge_glow", true) && Settings.canDrawOverlays(this)) {
                        try {
                    ...
                    if (actions != null) {
                        for (action in actions) {
                            ...
                            if (actionTitle.contains("decline") || actionTitle.contains("hang up") || actionTitle.contains("reject")) {
                                try {
